...
csv_path = "TATAMOTORS.NS_stock_data.csv"  # Ensure file is updated
window_size = 120
...
model_path = os.path.join(log_dir, f"final_GRU_model_{timestamp}.keras")
...
metrics_file = os.path.join(log_dir, f"metrics_GRU_{timestamp}.txt")
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(log_dir, f"loss_curve_GRU_{timestamp}.png"))
plt.close()
...
plt.savefig(os.path.join(log_dir, f"metrics_comparison_GRU_{timestamp}.png"))
plt.close()
